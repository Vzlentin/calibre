"""VN2 simulator cost diagnostics."""
...
def optimal_order_path_for_sku(
    ...
    *,
    ...
    if total_weeks - decision_rounds <= lead_time:
        return just_in_time_order_path_for_sku(
            initial_state,
            demand_by_week,
...
def just_in_time_order_path_for_sku(
    initial_state: ProductState,
    demand_by_week: Mapping[int, float],
    *,
    decision_rounds: int,
...
def oracle_diagnostic(
    *,
    ...
    for uid, state in cache.initial_states.items():
        ...
        sku_orders = optimal_order_path_for_sku(
            state,
            sku_demands,
            decision_rounds=decision_rounds,
            total_weeks=total_weeks,
    ...
    oracle_simulator = simulate_orders(
    ...
    by_sku, by_round, totals = cost_diagnostic_tables(actual, oracle_summary, oracle_history)
