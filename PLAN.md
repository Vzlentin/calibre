...
| Events calendar | regressor columns in `future_x` (`add_calendar_features` for global ML) | clean |
...
| Hierarchy / censoring | typed in `DatasetBundle` but optional | typed, engine does not yet consume hierarchy |
...
- **Flat task list — no SKU batching or grouping.** Tasks fan out
  one-per-unique_id. Ray absorbs the count, but there's no higher-level
  grouping (e.g., "run category A first"). The task list is flat.
...
- [[architecture]] — seven Protocols + forecast-frame contract
- [[vision]] — Oventi white-box vs SaaS positioning
- [[cloud-native-strategy]] — Track 2 audit (2026-05-13)
- [[2026-05-17-cloud-native-roadmap]] — canonical Phase 0–8 plan
- [[2026-05-18-stack-decision-and-ray-migration]] — Ray rationale + PR #32
...
**Goal:** Make cost-objective HPO correct, distributed, and mode-aware.

**Files:** `calibre/tuning/optimizer.py`, `calibre/tuning/objectives.py`, `calibre/storage/state.py`

**Changes:**
- (a) Remove the Ray Tune sequential fallback when `conformal_runtime_factory` is set. Snapshot conformal state to a per-trial object-store URI before launching Ray Tune; hydrate via `SymmetricIntervalRuntime.from_state` inside `_trainable`.
- (b) Accumulate cost across origins in both sequential (`_evaluate_candidate`) and Ray Tune (`_trainable`) paths. Report running mean (`total_cost / origin_idx`) as the intermediate metric for ASHA pruning; return final `total_cost` as the trial objective.
- (c) Make `Cost.evaluate` mode-aware. In `perhorizon` mode, evaluate cost per-horizon and sum. In `cumulative` mode, assert a single window and keep current semantics.

**Tests:**
- `test_tune_conformal_fallback_removed`
- `test_cost_accumulates_across_origins`
- `test_cost_perhorizon_vs_cumulative`

**DoD:** `uv run pytest tests/test_cost_objective_aggregation.py tests/integration/test_ray_tune_conformal.py` green.

---

### Phase 2 · State Keying + Session Identity + InventoryAdapter

**Goal:** Move conformal state from run-scoped single blobs to per-partition session-scoped rows; introduce inventory loading as a first-class Protocol.

**Files:** `calibre/storage/models.py`, `calibre/storage/state.py`, `calibre/execution/backend.py`, `calibre/conformal/runtime.py`, `calibre/simulation/`, Alembic migration

**Changes:**
- Replace hard-coded `RUNTIME_PARTITION = "__runtime__"` with real per-(uid, model, horizon) partition strings in `upsert`/`get`.
- Add `session_id: UUID` to `conformal_state` schema. `run_id` becomes an audit pointer in the `runs` table.
- Add `PendingStore` Protocol and `SqlPendingStore` implementation; migrate `DecisionLoop.pending` from `list[pd.DataFrame]` to persistent table `pending_observations`.
- Add `InventoryAdapter` Protocol (`load_state`, `load_lead_times`). Backtests use `SyntheticInventoryAdapter`; production uses injected adapter.
- Add TTL/compaction on `conformal_state` rows.

**Tests:**
- `test_state_resume_across_runs`
- `test_per_partition_state_rows`
- `test_pending_persists_across_invocations`
- `test_synthetic_vs_injected_inventory`

**DoD:** `uv run pytest tests/test_state_resume.py tests/test_inventory_adapter.py` green; Alembic migration applies cleanly.

---

### Phase 3 · Global Model Fan-Out + API Lifecycle Split

**Goal:** Remove the global-model driver bottleneck; split the monolithic `/forecasts` endpoint into lifecycle-aware endpoints.

**Files:** `calibre/execution/backend.py`, `calibre/api/main.py`, `calibre/api/schemas.py`, `calibre/cli/commands.py`

**Changes:**
- Wrap each global task config in its own `@ray.remote` call so ensemble members fan out. Per-uid local fan-out stays unchanged.
- Split API surface:
  - `POST /fit` — async, returns `fit_handle`
  - `POST /predict` — sync, takes `fit_handle` + origin, returns forecast frame
  - `POST /order` — sync, takes forecast frame + inventory snapshot + costs, returns `order_ledger`
  - `POST /observe` — async, takes `session_id` + actuals, updates conformal state
  - `GET /sessions/{tenant}/{uid}` — returns state + last forecast + open orders
- Skip `/tune` and `/calibrate` for a follow-up; they need Phase 1.

**Tests:**
- `test_global_models_fan_out`
- `test_predict_endpoint`
- `test_order_endpoint`
- `test_observe_persists_state`

**DoD:** `uv run pytest tests/test_api_lifecycle.py tests/test_global_fan_out.py` green.

---

### Phase 4 · Unified Search Space + Cache + Drift Metric

**Goal:** Enable joint HPO over model, conformal, and ordering configs; add model artifact caching and conformal coverage drift alerts.

**Files:** `calibre/tuning/task.py`, `calibre/tuning/optimizer.py`, `calibre/forecasting/cache.py` (new), `calibre/core/metrics.py`

**Changes:**
- Introduce `TuningCandidate(model_config, conformal_config, ordering_config)` dataclass. Change `search_space` return type accordingly.
- Add `Regret(decision_rule, arithmetic, costs)` as a `TuningObjective` sibling to `Cost` and `Pareto`.
- Add `ModelArtifactCache(uri)` keyed by `adapter.cache_key(task)`. `adapter.fit` checks cache before training.
- Add `calibre_conformal_coverage_drift{model, partition}` gauge derived from `AdaptiveAlphaController.error_history` running mean minus target.

**Tests:**
- `test_unified_search_space`
- `test_regret_objective`
- `test_model_cache_hit`
- `test_coverage_drift_metric`

**DoD:** `uv run pytest tests/test_unified_search_space.py tests/test_model_cache.py tests/test_drift_metric.py` green.
