...
from calibre.storage.objstore import canonical_ledger_uri
...
def test_run_store_contract_status_and_idempotency(store: RunStore) -> None:
    ...
    assert succeeded.row_count == 7


def test_sql_run_store_records_artifacts(tmp_path) -> None:
    ...
    parsed = UUID(run.id)
    with session_scope(store.factory) as session:
        ForecastPointerRepo(session).upsert(
            parsed,
            "ledger",
            canonical_ledger_uri(ledger_path.as_posix()),
            int(ledger_path.stat().st_size),
        )
