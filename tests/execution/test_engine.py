...
from calibre.execution.ledger import InMemoryLedger, InMemoryOrderLedger, OrderLedger
...
# tests above; here we pin the contract of each individual phase.
# ---------------------------------------------------------------------------
...
def _materialize_refs_for(engine, tasks):
    ...
    Returns ``(parallel_refs, direct_refs)`` for a pre-partitioned task list so
...
def test_predict_phase_concatenates_local_and_global():
    ...
    engine = BackendEngine()
    for parallel_refs, direct_refs in _materialize_refs_for(engine, [local_task, global_task]):
...
def test_calibrate_phase_applies_intervals():
    ...
    engine = BackendEngine(conformal=ConformalOptions(runtime=runtime))
    for parallel_refs, direct_refs in _materialize_refs_for(engine, [task]):
