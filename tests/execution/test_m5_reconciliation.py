...
The seam runs on real M5 hierarchy attributes and is a strict no-op on the
bottom-only fixture frame (strategy *divergence* awaits aggregate base
forecasts — a documented follow-up). It is also byte-identical on the
...
def _m5_bundle_tasks_origins():
    ...
    tasks = build_tasks(bundle.history, model_configs, 1)
    origins = config.origins.to_list()
    return bundle, tasks, origins


def _run_m5(bundle, tasks, origins, reconciler) -> pd.DataFrame:
...
def test_summing_matrix_built_from_real_m5_attributes() -> None:
    bundle, _tasks, _origins = _m5_bundle_tasks_origins()
...
def test_m5_bottom_up_run_completes_and_reconcile_executes() -> None:
    bundle, tasks, origins = _m5_bundle_tasks_origins()
    spy = _CountingReconciler(BottomUpReconciler())
    ledger = _run_m5(bundle, tasks, origins, spy)
...
def test_m5_bottom_yhat_identical_to_noop_run() -> None:
    """Bottom-only data => bottom_up is identity on y_hat vs a none-strategy run."""
    bundle, tasks, origins = _m5_bundle_tasks_origins()
    keys = [UNIQUE_ID, FORECAST_ORIGIN, MODEL_NAME, H]

    none_run = _run_m5(bundle, tasks, origins, NoOpReconciler()).sort_values(keys)
    bottom_up_run = _run_m5(bundle, tasks, origins, BottomUpReconciler()).sort_values(keys)

    np.testing.assert_array_equal(
        bottom_up_run[Y_HAT].to_numpy(),
        none_run[Y_HAT].to_numpy(),
    )
...
def test_m5_conformal_interval_columns_present_after_reconcile() -> None:
    ...
    bundle, tasks, origins = _m5_bundle_tasks_origins()
    ledger = _run_m5(bundle, tasks, origins, BottomUpReconciler())
